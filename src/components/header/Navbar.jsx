      ...
      <ul className="flex flex-col w-full gap-2">
        {navbars.map((nav, index) => (
          <li
            key={index}
            className={clsx(
              `text-white cursor-pointer hover:text-black border border-none font-semibold hover:bg-orange-100 px-2 py-1 rounded-md`,
              pathname === nav.href &&
                "hover:bg-orange-400 border px-2 py-1 rounded-md bg-orange-400  hover:text-white"
            )}
          >
            <Link to={nav.href} className="text-[1.6rem]">
              {nav.label}
            </Link>
          </li>
        ))}
